...
// Pinning a pod's address with an annotation.
//
...
//	networks-spec has no primary entry      -> fall back to cni.cilium.io/ipAddress
//	networks-spec is not a JSON list        -> fail the allocation
...
func (ipam *IPAM) resolveRequestedIP(owner string, family Family) (netip.Addr, error) {
	...
	value, source, err := requestedAddressOf(pod)
	if err != nil {
		return netip.Addr{}, fmt.Errorf("pod %s: %w", owner, err)
	}
	if value == "" {
		return netip.Addr{}, nil
	}
...
// networkSpec is one entry of the annotation.PodAnnotationNetworksSpec list.
// Only the fields that decide an address are decoded; an entry describes a
// whole network attachment and carries more than this.
type networkSpec struct {
	Type      string `json:"type"`
	IPAddress string `json:"ipAddress"`
}

// clusterNetworkPrimaryCNIType marks the entry in the networks-spec list that
// describes the address this CNI is expected to hand out. Entries of any other
// type belong to additional networks that something else attaches, and are not
// this allocator's business.
const clusterNetworkPrimaryCNIType = "Main"

// requestedAddressOf returns the address a pod pins, and the annotation it came
// from so that an error can name it.
//
// Two annotations can carry the request. networks-spec is the richer one, a
// list describing every network attached to the pod, and it wins: a pod that
// has one is managed by something that knows about all of its networks.
// cni.cilium.io/ipAddress is the older single-valued form and remains the
// fallback -- including when networks-spec is present but says nothing about
// the primary network, which is a pod whose other networks are specified and
// whose primary address is not.
//
// A networks-spec that cannot be parsed is an error rather than a fallback. It
// is a request that was made and cannot be read, and the one thing this code
// must never do is silently hand out a different address than the one asked
// for.
func requestedAddressOf(pod k8sTables.LocalPod) (value, source string, err error) {
	if spec := pod.Annotations[annotation.PodAnnotationNetworksSpec]; spec != "" {
		var networks []networkSpec
		if err := json.Unmarshal([]byte(spec), &networks); err != nil {
			return "", "", fmt.Errorf("annotation %s: %q is not a list of network specs: %w",
				annotation.PodAnnotationNetworksSpec, spec, err)
		}
		for _, n := range networks {
			if n.Type == clusterNetworkPrimaryCNIType && n.IPAddress != "" {
				return n.IPAddress, annotation.PodAnnotationNetworksSpec, nil
			}
		}
	}

	if v := pod.Annotations[annotation.PodAnnotationIPAddress]; v != "" {
		return v, annotation.PodAnnotationIPAddress, nil
	}
