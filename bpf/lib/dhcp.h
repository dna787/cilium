...
#ifndef DHCP_INTERFACE_MTU
	/* MTU of the veth interface moved into the pod */
	#define DHCP_INTERFACE_MTU       1500
#endif
...
dhcp_prepare_options(__u8 request_type, void *options, void *data_end)
{
	...
	/* option value is a 16 bit unsigned integer in network byte order */
	__u8 iface_mtu[] = {
		DHO_INTERFACE_MTU,
		2,
		(__u8)((DHCP_INTERFACE_MTU >> 8) & 0xFF),
		(__u8)(DHCP_INTERFACE_MTU & 0xFF)
	};
	...
	COPY_DATA_IF_SAFE(options, iface_mtu, data_end);
...
dhcp_prepare_response(struct __ctx_buff *ctx, __u8 request_type)
{
	...
	/* 1.20 replaced THIS_INTERFACE_MAC with the typed config object; this is
	 * the host side MAC of the endpoint's device, which the reply is sourced
	 * from. Same idiom as bpf_lxc.c's ARP responder.
	 */
	union macaddr node_mac = CONFIG(interface_mac);
	/* 1.20 replaced the per-endpoint LXC_* defines with the typed config
	 * object; the endpoint's own address is what we hand back as yiaddr.
	 * Same idiom as bpf/lib/lxc.h.
	 */
	__u32 yiaddr = CONFIG(endpoint_ipv4).be32;
	/* manually calculated, based on dhcp_prepare_options implementation */
	__u16 dhcp_options_len = 3 + 6 + 6 + 6 + 6 + 6 + 4 +
		sizeof(domain_search_list) + 1;
	...
	/* ctx_full_len() returns __u64. The clang shipped with the 1.20 builder
	 * image diagnoses the narrowing that older clang accepted, and the
	 * datapath is built with -Werror, so cast explicitly. A packet length
	 * never exceeds __u32.
	 */
	__u32 len_old = (__u32)ctx_full_len(ctx);
